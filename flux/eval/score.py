...
def score_one(image_path: Path, lora: str, prompt_id: str, seed: int, scorer: str) -> dict | None:
    ...
    for axis in AXES:
        while True:
            raw = input(f"  {axis:12s} (1–5, Enter=skip): ").strip()
            if raw == "":
                print("  Skipped.")
                return None
            try:
...
def print_leaderboard(scores: list[dict]):
    ...
    print(f"\n{'LoRA':<16} {'N':>4} {'Mean':>6} {'Props':>6} {'Hands':>6} {'Feet':>6} {'Mouth':>6} {'Integ':>6}")
    print("─" * 70)
    rows = []
    for lora, entries in by_lora.items():
        ...
        axis_means = {}
        for axis in AXES:
            vals = [e["scores"].get(axis) for e in entries if e["scores"].get(axis)]
            axis_means[axis] = sum(vals) / len(vals) if vals else 0
        rows.append((mean, lora, n, axis_means))
    rows.sort(reverse=True)
    for mean, lora, n, axis_means in rows:
        print(
            f"{lora:<16} {n:>4} {mean:>6.1f}"
            f" {axis_means['proportions']:>6.1f}"
            f" {axis_means['hands']:>6.1f}"
            f" {axis_means['feet']:>6.1f}"
            f" {axis_means['mouth_teeth']:>6.1f}"
            f" {axis_means['integument']:>6.1f}"
        )
    print()
